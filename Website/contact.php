<?php
    ...
    $stmt = $conn->prepare("INSERT INTO contact_form (name, email, message, subscribe) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("sssi", $name, $email, $message, $subscribe);

    // Execute the statement and handle the results
    if ($stmt->execute()) {
        // Success message
        echo '<h5>Thanks for contacting Franklin\'s!</h5>';
        echo '<p>Please allow 24 hours for a response.</p>';
        echo '<p><a href="/final" class="button block">&laquo; Go to Home Page</a></p>';
    } else {
        // Error message
        echo '<h4 class="error">Error occurred: ' . $stmt->error . '</h4>';
        echo '<a href="contact.php" class="button block">Go back and try again</a>';
    }
